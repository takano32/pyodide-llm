...
#
#   python3 tests/perplexity_native.py <out of tests/perplexity_prepare.py> <tokens> <text file>
import json
import sys
import time
...
out, count, text = sys.argv[1], int(sys.argv[2]), Path(sys.argv[3]).read_text()
...
print(json.dumps({"model": Path(out).name, "backend": llama.backend, "perplexity": value, "tokens": n,
