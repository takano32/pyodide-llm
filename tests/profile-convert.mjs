...
//
//   node tests/profile-convert.mjs <directory with config.json, model.safetensors and the tokenizer> [int8|int6|float32]
//
...
const conversion = convert.Conversion.callKwargs(new TextDecoder().decode(range(8, 8 + headerBytes)), 8 + headerBytes,
  fs.readFileSync(`${dir}/config.json`, "utf8"), new Uint8Array(fs.readFileSync(`${dir}/${tokName}`)), tokName, { dtype, start: 8 + headerBytes, quantize_rows: quantizeRows });
