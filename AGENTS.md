...
- **スレッドで伸びるのは約 2 倍、頭打ちはコア数ではなくメモリ帯域（2026-09-21 に測り直した）。** バッチ 1 の int8 は重みを 1 回読んで 1 回掛けるだけなので、**1 バイトあたり 1 積和**。だから「1 コアの演算の速さ」と「メモリの上限」の比がそのまま上限になる。この開発機（C + NEON、コアを固定して計測）: 大コア（A78）1 つで 11.9 G 積和/秒、2 つで 21.5、3 つで 23.5、4 つで 21.9。読み出しだけなら 1 コアでもう 20.3GB/s 出て、上限は 21〜24GB/s。**11.9 と 23 の比がそのまま 2 倍**で、3 コア目からは増えない。小コア（A55）は 1 つ 3.3 G 積和/秒。**大小のコアに均等に割ると 2 つの大コアより遅い**（全 8 コアに 4 スレッドで 12.8、8 スレッドで 13.6、16 スレッドで 18.3 G 積和/秒。待ち合わせが小コアに引きずられる）。ついでに分かったこと: **単スレッドのネイティブ（NEON）11.9 と、WASM SIMD カーネルの 10.7〜12.0 はほぼ同じ**。カーネルはネイティブと同じ速さまで来ている。**それでもこのプロジェクトでは使えない**: GitHub Pages は COOP/COEP を送れないので `SharedArrayBuffer` が無く、ブラウザにスレッドが無い（方針 6）。理由は「効かないから」ではなく置き場所の制約。計測は使い捨ての C（`pthread` + `taskset`、192MB の行列 × ベクトル）で、リポジトリには入れていない。
