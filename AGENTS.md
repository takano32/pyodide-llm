...
- **OS 5 種 × ブラウザ 5 種、23 通りすべてで 4 モデルが動く（T44・T58、`browsers.yml`、表は `kernels/README.md`）。** Linux と Windows（x86-64 と ARM）、macOS（Apple M1）で、Playwright の Chromium・Firefox・WebKit と、ランナーに入っている本物の Chrome・Edge。WebKit はどの OS でも relaxed SIMD が無く、int8 は `matmul_q8` で動く。**Firefox はどの OS・どちらのアーキテクチャでも Chromium 系の 8〜15 分の 1**（同じランナーで llm-jp-3-150m が 13 対 107 tok/s）で、開発機だけの現象ではない（原因は未調査、T59）。Windows ARM では Playwright の Chromium と WebKit が x86-64 のエミュレーションで動くので 3 分の 1 の速さ。ランナーは共有の仮想マシンで CPU もまちまちなので、OS をまたいだ速度の比較には使わない。Safari そのものと iPhone の実機は未確認。
