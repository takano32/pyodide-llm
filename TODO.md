...
### T58 できるだけ多くの OS とブラウザで CI テストする — 状態: 進行中（担当: Fable。持ち主の依頼: できる範囲でたくさん、重ければランナーを分ける）
- 内容: T44 の `webkit.yml` を `browsers.yml` に広げた。本番サイトに対して、OS 5 種（Linux x86-64 / Linux ARM / Windows x86-64 / Windows ARM / macOS Apple silicon）× ブラウザ 5 種（Playwright の Chromium・Firefox・WebKit と、ランナーに入っている本物の Chrome・Edge）= 23 ジョブ（Linux ARM には Chrome と Edge が無い）。1 ブラウザ = 1 ジョブなので並列に走る。手動（`gh workflow run browsers.yml`）か週 1 回。各ジョブが 4 モデルの backend と tok/s を Summary に書く。
- 1 回目（4 ジョブ）で分かったこと: (1) 本物の Edge は `favicon.ico` の 404 をコンソールのエラーとして報告し、テストが失敗と判定した（モデルは 4 つとも動いていた）→ ページにファイルなしのアイコン（data URI の SVG）を付けた。(2) **Firefox は x86-64 の Linux でも Chromium の約 8 分の 1**（llm-jp-3-150m 13.3 対 106.9 tok/s、同じランナー）。開発機（ARM）だけの現象ではなく Firefox の性質。原因は未調査（→ 候補 T59）。
- 残り: 23 ジョブの結果を見て、起動できない組み合わせを matrix から外すか「動かない」と記録する。結果の表を AGENTS.md と `kernels/README.md` に載せる。
...
- 調べること: `tests/profile.mjs firefox` で 1 トークンの内訳と行列積のスループットを取り、Chromium と比べる。カーネルの中が遅いのか（SpiderMonkey の SIMD / relaxed SIMD のコード生成）、ctypes の呼び出し（Pyodide が関数ポインタ呼び出しに使う JS のトランポリン）が遅いのか、Python のインタプリタ全体が遅いのか（wasm の例外処理やベースラインコンパイラ）で、打てる手が変わる。
