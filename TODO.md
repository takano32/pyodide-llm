...
- 未着手のタスクは、依存のあるものを除いて単独で着手できる。勧める順番: 計測の道具立ては T52（スイッチ）→ T45（ベンチ。T43 を吸収）。T44（WebKit の CI）は独立で、並列に進められる。T48（記事）はいつでも。性能は T54 で一区切り（int8 の行列積は V8 の命令の速さの 9 割に達した）。T47 は既定モデルで最大 7% なので優先度 低。T39 は後回しでよい。
