...
### T64 設定で切り替わる小さな差分（bias など → Qwen2.5 が動く）— 状態: 未着手（2026-09-20 採用。T61 を分割した 2 つめ。T63 の後。規模 中、Fable 向き）
...
### T65 別系統の forward（GPT-2 系）— 状態: 未着手（2026-09-20 採用。T61 を分割した 3 つめ。T64 の後。着手の前に T64 の歯止めの条件を満たすか確かめ、満たさなければ持ち主に相談する）
...
### T62 プロジェクト名を `pyodide-llm` に変える — 状態: 未着手（2026-09-20 採用。名前は `pyodide-llm` に決定。時期は T64 が終わった時点で、1 回で済ませる）
