...
### T60 Hugging Face から検証済みのモデルを取得して、変換して動かす — 状態: 候補（持ち主の発案。Fable の意見: やる価値が大きい。規模 中〜大、Fable 向き）
...
- UI の意見: **スイッチは足さず、コンボボックスの中を `<optgroup>` で分けるのがよい**（「このサイトのモデル」と「Hugging Face から取得して、このブラウザで変換」）。理由: 常に見える要素が増えない（いまフォルダ・設定・送信の 3 つ）、選択肢が見えているほうが見つけてもらえる、サイズを項目名に書けば「大きなダウンロードになる」ことは伝わる。スイッチに意味があるのは「第三者のサーバーへ取りにいくことへの同意」を明示したい場合で、そのときは設定のパネル（T35）の中に置く。
- 設計の要点:
  ...
  4. 指示モデルにはチャットテンプレートが要る（T46）。T46 を先にやるか、一緒にやる。
...
### T46 指示に応える日本語モデル + 会話の継続 — 状態: 候補（提案を変更: 会話の継続は却下、テンプレートは T60 の付属としてだけ）
- 2026-09-19 の再検討（持ち主: 実験の範囲から出る気がする。Fable: 同意。提案を「採用」から下の形に改める）: このプロジェクトの価値は「WASM Python でどこまでできるかを測って公開する」ことで、チャットとして使えるかどうかではない。T46 は 2 つに分けて考える。
  - **(b) 会話の継続（ターンをまたぐ KV キャッシュ、文脈があふれたときの作り直し、リセット）は範囲の外。却下を提案。** 状態を持つ UI の作り込みで、測れるものが増えない。150M の指示モデルの受け答えは弱く、チャットボットとして評価される土俵に自分から上がることになる。
  - **(a) チャットテンプレート（1 ターンだけ）は、単独ではやらない。** T60 を採用して HF の一覧に指示モデルを入れる場合にだけ、その付属として入れる（入力をテンプレートに入れて渡すだけで、20 行ほど。状態は持たない）。T60 をやらないなら (a) もやらない。
  - 「チャット風の UI なのに続きを書くだけ」という違和感は、機能ではなく文言で解く（イントロに「入力した文の続きを書きます」と書く程度）。
  - 実験の側に残る論点: 長いプロンプトの処理速度。入力欄は複数行になり、コンテキストは 4096 になったので、長文を貼れば prefill の遅さは見える（回答の下の内訳に prompt の tok/s が出る）。T49（prefill のバッチ化）を却下した理由は「プロンプトが数トークンだから」だったので、長い入力が普通になるなら T49 のほうを見直す。会話機能は要らない。
