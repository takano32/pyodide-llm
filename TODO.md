...
- 未着手のタスクは、依存のあるものを除いて単独で着手できる。勧める順番: T52（スイッチ）→ T45（ベンチ。T43 を吸収）→ T47（まず計測）。T44（WebKit の CI）は独立で、並列に進められる。T48（記事）はいつでも。T39 は後回しでよい。
...
### T52 最適化を個別に切るスイッチ — 状態: 未着手（持ち主の発案。T45 と T47 より先にやると、どちらも楽になる）
...
### T47 Python の呼び出し回数を減らす（層ごとに 1 回のカーネル呼び出し）— 状態: 未着手（2026-09-19 採用。まず計測。規模 中、Fable 向き。T52 の後）
- 目的: 1 トークンで約 100 回カーネルを呼んでいて、呼び出し 1 回は 3〜12µs（実測）。合計で 1 トークンあたり 0.3〜1.2ms。「インタプリタのコストはいくらか」を数値で出すのは、この実験の核心。
- 効く範囲の見積もり（未計測）: stories260K（1 トークン約 1ms）は大半が呼び出しのコストで数倍の余地、tiny-lm（約 3.6ms）は 1〜3 割、**既定の llm-jp-3-150m（約 16ms）は 3% 前後で体感は変わらない**。候補の中で tok/s に効くのはこれだけだが、効くのは小さいモデル。
- 既定モデルはメモリ帯域で頭打ちの可能性がある（未計測）: int8 の llm-jp は 1 トークンで重みを約 160MB 読む（層 約 85MB + 分類器 約 76MB）。62 tok/s なら毎秒約 10GB で、スマホ級 SoC のシングルスレッドの実効帯域に近い。そうならカーネルを磨いても伸びず、効くのは 1 トークンで読むバイト数を減らすこと（int4 系）だけだが、int4 は品質 +16.8% で却下済みで、分類器が帯域の半分を占めるので FFN だけ int4 にしても 1.2〜1.3 倍。勧めない。
- 手順:
  1. **まず計測する**（半日以下、コードはほとんど増えない）。llm-jp-3-150m と tiny-lm で、1 トークンの時間の内訳（Python にいる時間 / 層のカーネル / 分類器 / サンプリング）を Node 上の Pyodide と Chromium で測る。あわせて、キャッシュに載る小さい行列と載らない大きい行列で `matmul_q8` のスループット（毎秒の積和数）を比べ、帯域律速かどうかを判定する。結果は `kernels/README.md` と README の Measurements に載せる（どちらの結果でも載せる価値がある）。
  2. 計測の結果、小さいモデルで呼び出しのコストが見積もりどおりなら、層 1 つぶんの処理（rmsnorm → q/k/v → RoPE → attention → o → 残差 → FFN）を 1 回の呼び出しにまとめたカーネルを足す。ポインタは層ごとの構造体（NumPy の配列に並べたアドレス）で渡す。「Python が層を順に呼ぶ」設計は保つ（1 トークン 12 回程度）。float32 と int8 の両方。既存の細かいカーネルは NumPy との比較と GQA のテストのために残す。
  3. **T52 のスイッチの 1 つにする**（層ごとの呼び出しを切ると、いまの細かい呼び出しに戻る）。これで効果の数値は T45 のベンチからも自動で取れる。
  4. 帯域律速でなかった場合は、カーネルの改善（アンロール、2 行同時処理など）を新しい候補として立てる。
