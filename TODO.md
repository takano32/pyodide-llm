...
- 未着手のタスクは、依存のあるものを除いて単独で着手できる。勧める順番: 性能は T54（カーネルの演算）。T47 はその後に採否を見直す。計測の道具立ては T52（スイッチ）→ T45（ベンチ。T43 を吸収）。T44（WebKit の CI）は独立で、並列に進められる。T48（記事）はいつでも。T39 は後回しでよい。
...
### T54 カーネルの演算を速くする（int8 の行列積と attention）— 状態: 未着手（T53 の結論。性能に効くのはここ。規模 中、Fable 向き）
- 根拠（T53 の実測、`kernels/README.md`）: 既定の llm-jp-3-150m は 1 トークンの 9 割が行列積（層 52%、分類器 38%）。int8 の行列積は帯域律速ではなく演算律速（キャッシュに載る行列でも載らない行列でも 10.7〜12.0 G 積和/秒、13GB/s。同じメモリで float32 のカーネルは 20GB/s を流す）。attention は位置 248 で 1 トークンに 3.7ms を足していて、積和あたりのコストが行列積の数倍。
- 手順（1 つずつ `node tests/profile.mjs` で測り、効かなかったものは数値を書いて戻す）:
  1. `matmul_q8r`（`kernels/kernel_relaxed.ts`）の内側のループ: グループ 2 つぶんのアンロールとアキュムレータの分離、スカラーで足している補正項（`corr`）のベクトル化、2 行同時の処理。V8 が relaxed dot を ARM64 でどう落としているか（SDOT を使うか）も調べる。同じ工夫を `matmul_q8`（relaxed SIMD なしの経路、Safari 用）にも入れる。目安: 帯域が律速になるまでに最大 1.5 倍の余地。
  2. `attention`（`kernels/kernel.ts`）: スコアの計算と重み付き和の SIMD 化の見直し、softmax の `fexp` を 4 個ずつ（T34 で作った `vexp` を使う）、ヘッドごとの作業の無駄を探す。目安: 位置 248 での上乗せ 3.7ms を半分以下に。
  3. 分類器（1 トークンの 38〜54%）は行列積そのものなので 1 の効果がそのまま出る。それ以上の手（語彙の一部だけを計算するなど）は出力が変わるのでやらない。
- 完了条件: float32 は NumPy と一字一句同じ出力のまま（`smoke.mjs`）。int8 は同じシードで変更前と同じ文（変わるなら理由を調べて記録）。llm-jp-3-150m と tiny-lm の tok/s（`profile.mjs` と、Chromium の e2e の 256 トークン）を変更前後で記録。
