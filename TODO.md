...
### T58 Linux の Chromium、Windows の Edge などを CI でテストする — 状態: 進行中（担当: Fable。持ち主の依頼）
- 内容: T44 の `webkit.yml` を `browsers.yml`（ブラウザの一覧）に広げる。matrix は macOS の WebKit、Linux（x86-64）の Chromium と Firefox、Windows の Edge（ランナーに入っている本物の Edge を Playwright の `channel: "msedge"` で起動）。手動か週 1 回、本番サイトに対して 4 モデル。結果（backend と tok/s）は各ジョブの Summary。ランナーは共有の仮想マシンなので、速度は参考程度。
