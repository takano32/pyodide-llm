...
- 未着手のタスクは、依存のあるものを除いて単独で着手できる。勧める順番: 計測の道具立ては T52（スイッチ）→ T45（ベンチ。T43 を吸収）。T48（記事）はいつでも。性能は T54 で一区切り（int8 の行列積は V8 の命令の速さの 9 割に達した）。T47 は既定モデルで最大 7% なので優先度 低。T39 は後回しでよい。
...
### T56 既定でモデルを最大限に活用する — 状態: 未着手（持ち主の依頼。十分に速くなったので、既定値で手加減しない。規模 小〜中）
- 目的: 速度に余裕ができた（llm-jp-3-150m が 75〜79 tok/s）ので、遅かった頃に入れた「既定値での手加減」をやめる。最初は「Max tokens の既定値を最大にする」として登録したが、持ち主の意図はもっと広い: 何も設定しなくても、モデルの能力をいっぱいまで使う。
- いまある手加減と、やること:
  1. **生成の長さ。** 既定は 256 トークン（`src/models.js` の `steps: 256`）で、コンテキストより手前で文章が切れる。既定を最大にする。エンジンは `steps <= 0` をコンテキストの長さ（`seq_len`）として扱うので `steps: 0` でよい。ローカルのモデル（T38・T42）の既定値も同じ。設定のパネル（T35）は、既定のときスライダーを右端にして数値（例 `512`）を表示し、最大のままなら「既定値と違う」の点を付けない（0 と `seq_len` を同じものとして比べる）。停止トークンで自然に終わるモデルはこれまでどおり早く終わり、途中でやめたければ停止ボタン（T24）がある。
  2. **コンテキストの長さ。** llm-jp-3-150m は本来 4096 トークンだが、ビルド時に 512 に切っている（Makefile の `convert_hf.py ... 512`。理由は KV キャッシュで、12 層 × 位置 × 512 × 4 バイト × 2 = 4096 トークンで約 200MB、512 なら 25MB）。ブラウザの中での変換（T42）の既定も 512。これを伸ばす。候補は 4096（本来の長さ）か、その手前（2048 で約 100MB、1024 で約 50MB）。**測ってから決める**: ヒープの大きさ（いま約 280MB。スマホで開けること）、長い位置での 1 トークンの時間（attention は位置に比例して重くなる。T54 の後で位置 248 の上乗せが 0.7ms なので、位置 4096 では 10ms 以上の上乗せになる見積もり。未計測）。KV キャッシュを必要になった分だけ確保する（位置が進んだら伸ばす）か、float16 で持てば、短い生成ではメモリを払わずに済む。tiny-lm は 2048 のままで切っていない。TinyStories も切っていない。
  3. ほかに既定値で手加減しているものが無いか、着手時に `src/models.js`・`Makefile`・`public/worker.js` を見直す（分かっているのは上の 2 つ）。「スマホでは軽いモデルから始める」のような分岐は、いまは無い（既定は常に llm-jp-3-150m）。
- 着手時に持ち主に確かめること: 2 のコンテキストをどこまで伸ばすか（計測値を見せて決めてもらう）。1 だけ先にやって 2 を別のコミットにしてよい。
- 注意: 長い生成は、位置が進むほど 1 トークンが重くなるので、256 トークンのときより tok/s が下がる。README と AGENTS.md の「256 トークン」での実測値は条件を書いたまま残し、新しい既定での数値を足す。`tests/e2e.mjs` は実行時間が延びるので、テストでは 256 に絞る（設定のパネルを操作するか、T43 のクエリパラメータを使う）。`src/models.js` の `bytes` は、コンテキストを変えても int8 の llm-jp では変わらない（int8 のファイルは RoPE の表を持たない）が、float16 の原本は RoPE の表のぶん大きくなるので直す。Cache API のキーにはサイズが入っているので、サイズが変われば古い部品は使われない。
- 完了条件: 設定を触らずに送信すると、停止トークンが出ない限りコンテキストいっぱいまで書く。llm-jp-3-150m のコンテキストが、決めた長さになっている。ヒープと、長い位置での tok/s の実測値を AGENTS.md と `kernels/README.md` に載せる。e2e が通る。

### T57 日本語モデルの既定のプロンプトを「これからの流行りは」にする — 状態: 未着手（持ち主の依頼。規模 極小）
- 内容: 入力欄に最初から入っている文（`src/models.js` の各モデルの `prompt`）を、日本語のモデル（llm-jp-3-150m、tiny-lm と、それぞれの原本）で「昔々、」から「これからの流行りは」に変える。プレースホルダー（`JAPANESE`、入力欄が空のときに出る説明）は変えない。（最初はプレースホルダーの変更として登録したが、持ち主の意図は既定値のほうだった。）
- 着手時に確かめること: 「昔々、」はテストや文書の各所に出てくる。`tests/smoke.mjs`・`tests/e2e.mjs`・`tests/profile_token.py` などは自前で「昔々、」を渡しているだけで、`models.js` の既定値には依存していないはず（依存していたら直す）。README の設定 JSON の例や TODO の過去の記録にある「昔々、」は、その時点の事実なので書き換えない。ページを開いて最初に出る文が変わるので、llm-jp-3-150m と tiny-lm で何回か生成して、書き出しとして不自然でないことを目で確かめる。
- 完了条件: 日本語のモデルを選ぶと入力欄に「これからの流行りは」が入っている。英語のモデル（"Once upon a time"）とローカルのモデルは変わらない。e2e が通る。
...
### T47 Python の呼び出し回数を減らす（層ごとに 1 回のカーネル呼び出し）— 状態: 未着手（優先度 低。T53 の結果、既定モデルで最大 7%、tiny-lm で最大 12%。T54 の後に採否を見直す）
