...
### T27 生成設定の表示 — 状態: 進行中（担当: Opus）
- 目的: どの設定で生成した回答なのかを回答ごとに残す。シードが見えれば、同じ条件での再現や int8 と原本の比較を人に伝えられる。**表示だけ**で、設定を変える UI は T35。
- 触るファイル: `src/pages/index.astro`（回答の下の `170 tokens · 251.8 tok/s` の行）、`public/worker.js`。
- 手順:
  1. シードが指定されていないときは、ページ側で乱数のシード（32 ビットの整数）を引いて Worker に渡す。エンジンは `generate(..., seed)` に対応済みなので、これで「実際に使ったシード」が分かる。
  2. 回答の下の行に temperature とシードを足す（greedy のモデルは `greedy` とだけ出す。シードは意味がないので出さない）。行が長くなるので、スマホ幅では折り返してよいが、ページ自体はスクロールさせない。
- 完了条件: 表示されたシードを `generate` に渡すと同じ文になる（確認は Worker に直接メッセージを送るか、T35 の後に画面から）。要素は増やさず、既存の行の延長に収める。
- T28 と同じ行を触るので、並列にはやらない（続けてやるか、まとめてやる）。

### T28 計測の表示 — 状態: 進行中（担当: Opus）
- 目的: 実験としての数値を見せる。数値はこのプロジェクトの主役なので、メニューの奥にしまわない。**表示だけ。**
- 内容: 回答ごとの値（最初のトークンまでの時間、プロンプト処理と生成それぞれの tok/s）は回答の下の行に足し、内訳はその行をタップして開く（`<details>`）。ロードごとの値（Pyodide のロード / モデルのダウンロード / `Llama()` の構築）は、準備完了時のステータス行か、その内訳に出す。`worker.js` で `performance.now()` を取り、`ready` と `done` のメッセージに載せる。エンジンの `stats` にプロンプト処理の時間を足す。
- 完了条件: 閉じていれば今とほぼ同じ見た目。tok/s が計測のせいで落ちない（tiny-lm で変更前の 95% 以上）。

### T35 生成設定の変更 — 状態: 未着手（T27 の後）
