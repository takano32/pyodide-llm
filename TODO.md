...
### T55 カーネルの経路（int8 の活性値）の品質を測る — 状態: 候補（提案: 採用。規模 小、Fable 向き）
- 背景（T54 で分かった）: int8 の品質（llm-jp-3-150m でパープレキシティ 22.76 → 22.69）は、重みだけを int8 にして NumPy で計算した値。SIMD カーネルの経路は活性値も int8（relaxed SIMD のときは 7 ビット）に丸めていて、その経路のパープレキシティは一度も測っていない。NumPy の経路との logit の差は最大 1.6〜1.7（語彙 99584 個の中の最大値）。文章は破綻していないが、数値は無い。
- 手順: 以前の計測と同じテキストで、NumPy の経路 / `matmul_q8`（8 ビットの活性値）/ `matmul_q8r`（7 ビット）のパープレキシティを Node 上の Pyodide で測る（T52 のスイッチがあれば切り替えはそれで）。悪化が大きければ、活性値のグループを小さくする・分類器だけ 8 ビットにするなどを検討する。
- 完了条件: 3 つの数値を README と `kernels/README.md` に載せる。
