...
### T44 Safari 系（WebKit）を CI でテストする — 状態: 候補（提案: 採用。規模 小、Opus で可）
...
### T45 ベンチマークモード — 状態: 候補（提案: 採用。規模 小〜中。T43 を吸収）
...
### T47 Python の呼び出し回数を減らす（層ごとに 1 回のカーネル呼び出し）— 状態: 候補（提案: 採用。規模 中、Fable 向き）
- 目的: 1 トークンで約 100 回カーネルを呼んでいて、呼び出し 1 回は 3〜12µs（実測）。tiny-lm（1 トークン約 3.6ms）ではその 1〜3 割が呼び出しのコストという見積もり（未計測）。「インタプリタのコストはいくらか」を数値で出すのは、この実験の核心。
- 手順: まず計測する（`kernel_forward` の 1 トークンの時間のうち、カーネルの中にいる時間と Python にいる時間）。見積もりどおりなら、層 1 つぶんの処理（rmsnorm → q/k/v → RoPE → attention → o → 残差 → FFN）を 1 回の呼び出しにまとめたカーネルを足す。ポインタは層ごとの構造体（NumPy の配列に並べたアドレス）で渡す。「Python が層を順に呼ぶ」設計は保つ（1 トークン 12 回程度）。float32 と int8 の両方。既存の細かいカーネルは NumPy との比較と GQA のテストのために残す。
...
### T48 ctypes + `dylink.0` の手法を単体の記事と最小サンプルにする — 状態: 候補（提案: 採用。規模 小、文章）
...
### T49 プロンプトの一括処理（prefill のバッチ化）— 状態: 候補（提案: 却下）
- 内容: プロンプトのトークンを 1 つずつでなく、まとめて行列 × 行列で処理する。最初のトークンまでの時間が縮む。
- 却下を提案する理由: このデモのプロンプトは数トークン（「昔々、」は 4 トークン、最初のトークンまで 0.03〜0.05 秒）で、効果が見えない。T46 で会話の履歴が長くなったら再検討の価値がある。

### T50 オフライン対応（PWA）— 状態: 候補（提案: 却下）
- 内容: Service Worker でページと Pyodide をキャッシュし、2 回目以降はネットワークなしで動く（モデルの部品はすでに Cache API にある）。
- 却下を提案する理由: 古い `worker.js` のキャッシュで 404 を出した前例があり（AGENTS.md）、Service Worker はその種の事故をもっと起こしやすい。「Pyodide は常に最新を実行時に解決する」方針とも噛み合わない。

### T51 スレッド化（SharedArrayBuffer）— 状態: 候補（提案: 却下）
- 内容: `coi-serviceworker` で GitHub Pages に COOP/COEP を付け、複数の Worker で行列積を分担する。
- 却下を提案する理由: 計測済みで、この規模のモデルではスレッドが効かない（ネイティブの OpenMP でも同じ）。Pyodide 自体がスレッドなしのビルドで、カーネルは Pyodide のメモリ（共有メモリではない）の上で動いている。方針 6（スレッドに頼らない）にも反する。
