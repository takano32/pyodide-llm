...
### T56 Max tokens の既定値を、そのモデルの最大にする — 状態: 未着手（持ち主の依頼。規模 小）
- 目的: いまの既定は 256 トークン（`src/models.js` の `steps: 256`）で、モデルのコンテキスト（llm-jp-3-150m は 512、tiny-lm は 2048、TinyStories は 256〜1024）より手前で文章が切れる。既定では最後まで書かせる。停止トークンで自然に終わるモデルはこれまでどおり早く終わり、途中でやめたければ停止ボタン（T24）がある。
- 手順:
  1. `src/models.js` の `steps` を「最大」を表す値にする。エンジンは `steps <= 0` をコンテキストの長さ（`seq_len`）として扱うので（`generate()`）、`steps: 0` でよい。ローカルのモデル（T38・T42）の既定値も同じにする。
  2. 設定のパネル（T35）: コンテキストの長さは `ready` で届く（`seq_len`）。既定のときはスライダーを右端にして、値の表示を数値（例 `512`）にする。「既定値と違う」の点は、最大のままなら付けない（0 と `seq_len` を同じものとして比べる）。モデルを切り替えたら新しいモデルの最大に戻る。
  3. 回答の下の行と計測（T27・T28）は、実際に生成したトークン数を出しているので変更なし。
- 注意: tiny-lm は 2048 トークンまで書けるようになる。位置が進むほど 1 トークンが重くなる（T54 で軽くなったが、ゼロではない）ので、長い生成の tok/s は 256 トークンのときより下がる。`tests/e2e.mjs` の実行時間も延びるので、テストでは設定で 256 に絞るか、T43（クエリパラメータ）を使う。README と AGENTS.md の「256 トークン」での実測値は、条件を書いたまま残す。
- 完了条件: 設定を触らずに送信すると、停止トークンが出ない限りコンテキストいっぱいまで書く。設定の表示と点の挙動が上のとおり。e2e が通る。
