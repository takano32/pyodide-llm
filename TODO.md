...
### T57 日本語モデルの入力欄のプレースホルダーを「これからの流行りは」にする — 状態: 未着手（持ち主の依頼。規模 極小）
- 内容: `src/models.js` の `JAPANESE`（いまは「文章の書き出しを入力（例: 富士山は、）」）を「これからの流行りは」に変える。対象は日本語のモデル（llm-jp-3-150m、tiny-lm と、それぞれの原本）。
- 着手時に持ち主に確かめること: プレースホルダーは入力欄が空のときにしか見えず、いまは既定のプロンプト「昔々、」が最初から入っているので、変えても普段は目に入らない。意図が「最初から入っている文」のほうなら、変えるのは各モデルの `prompt`（「昔々、」）。両方変える、または既定のプロンプトを空にしてプレースホルダーを見せる、という選択もある。`tests/smoke.mjs` と `tests/e2e.mjs` は「昔々、」に依存していない（モデルの既定のプロンプトをそのまま送るだけ）ことを着手時に確かめる。
- 完了条件: 日本語のモデルを選んで入力欄を空にすると「これからの流行りは」が出る（意図が既定のプロンプトなら、最初からその文が入っている）。英語のモデルは変わらない。
